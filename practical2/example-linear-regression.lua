...
feval = function(x_new)
   ...
   local inputs = sample[{ {2,3} }]:cat(torch.pow(sample[{ {2, 3} }], 2)):cat(torch.Tensor({sample[2] * sample[3]}))    -- slicing of arrays.
...
----------------------------------------
-- Test set 1e3 epochs.
-- current loss = 14.470808832888    
-- current loss = 14.441182100484    
-- id  approx     params 
-- 1   33.19 =   1.86 * 6 +  -0.26 * 4 +  23.10 * 1 
-- 2   40.36 =   1.86 * 10 +  -0.26 * 5 +  23.10 * 1   
-- 3   47.00 =   1.86 * 14 +  -0.26 * 8 +  23.10 * 1  
----------------------------------------
-- Test set 1e4 epochs.
-- current loss = 1.5816736089208   
-- current loss = 1.5816505966595   
-- id  approx     params    
-- 1   40.10 =   0.67 * 6 +   1.12 * 4 +  31.64 * 1    
-- 2   43.88 =   0.67 * 10 +   1.12 * 5 +  31.64 * 1  
-- 3   49.90 =   0.67 * 14 +   1.12 * 8 +  31.64 * 1 
-- ------------------------------------
-- Test set 1e5 epochs.
-- current loss = 1.3879817446763   
-- id  approx     params    
-- 1   40.33 =   0.65 * 6 +   1.12 * 4 +  31.98 * 1    
-- 2   44.03 =   0.65 * 10 +   1.12 * 5 +  31.98 * 1  
-- 3   49.96 =   0.65 * 14 +   1.12 * 8 +  31.98 * 1 
--------------------------------------
print('id  approx     params')
for i = 1,(#dataTest)[1] do
        local myPrediction = model:forward(dataTest[i])
        print(string.format("%2d  %6.2f = %6.2f * %d + %6.2f * %d + %6.2f * %d", i, myPrediction[1], x[1], dataTest[i][1], x[2], dataTest[i][2], x[3], 1))
end

----------------------------------
-- Least square closed normal form compared to 1e4.
-- current loss = 1.5817735645363	
-- current loss = 1.5817505191727	
-- id  approx     params	
--  1   40.10 =   0.67 * 6 +   1.12 * 4 +  31.64 * 1	
--  2   43.88 =   0.67 * 10 +   1.12 * 5 +  31.64 * 1	
--  3   49.89 =   0.67 * 14 +   1.12 * 8 +  31.64 * 1	
...
----------------------------------
-- Least square closed normal form compared to 1e5.
-- current loss = 1.3879805741527	
-- current loss = 1.3879803648071	
-- id  approx     params	
--  1   40.32 =   0.65 * 6 +   1.12 * 4 +  31.98 * 1	
--  2   44.03 =   0.65 * 10 +   1.12 * 5 +  31.98 * 1	
--  3   49.96 =   0.65 * 14 +   1.12 * 8 +  31.98 * 1	
-- Least square solution: 	
--   0.6501
...
Y = data:sub(1, -1, 1, 1)
X = torch.cat(data:sub(1,-1, 2, -1), torch.ones((#data)[1]))
...
print('Squared loss:', squared_loss/(#data)[1])

--------------------------------
-- Predictions:
X_test = torch.cat(dataTest, torch.ones((#dataTest)[1]))
pred = X_test * w
