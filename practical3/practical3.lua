...
for i = 1, iterations do
  ...
  if i % 10 == 0 then -- don't print *every* iteration, this is enough to get the gist
      test_losses[#test_losses + 1], test_misclass[#test_misclass + 1] = eval(test.data, test.labels)
      _, train_misclass[#train_misclass + 1] = eval(train.data, train.labels)
      eval_iter[#eval_iter + 1] = i
      print(string.format("minibatches processed: %6s, loss = %6.6f, test_loss = %6.6f", i, losses[#losses], test_losses[#test_losses]))
  end
end

-- Turn table of losses into a torch Tensor, and plot it
gnuplot.plot(
    {'train_loss', torch.range(1, #losses), torch.Tensor(losses), '-'},
    {'test_loss', torch.Tensor(eval_iter), torch.Tensor(test_losses), '-'}
)

gnuplot.plot(
    {'train_misclass', torch.Tensor(eval_iter), torch.Tensor(train_misclass), '-'},
    {'test_misclass', torch.Tensor(eval_iter), torch.Tensor(test_misclass), '-'}
)
